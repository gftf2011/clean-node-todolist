...
import { ControllerSpy } from '../../../../doubles/spies/app/controllers';
...
describe('Database Controller - Decorator', () => {
  it('should commit decoratee response', async () => {
    const decoratee = new ControllerSpy([Promise.resolve({ is: 'any' })]);
  ...
  it('should rollback and throw error', async () => {
    const decoratee = new ControllerSpy([Promise.reject(new Error('error'))]);
    const database = new PostgresTransactionSpy({
      createClient: {
        responses: [Promise.resolve()],
      },
