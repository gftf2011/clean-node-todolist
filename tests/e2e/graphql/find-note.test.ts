...
describe('Query - getNote', () => {
  ...
  it('should return 200 when note is found', async () => {
    ...
    expect(singleNoteResponse.body.data.getNote).toHaveProperty('id');
    expect(singleNoteResponse.body.data.getNote).toHaveProperty('finished');
    expect(singleNoteResponse.body.data.getNote).toHaveProperty('timestamp');
    expect(singleNoteResponse.body.data.getNote.title).toBe('any title');
  ...
  it('should return 400 if note is not found', async () => {
    ...
    const response = await serverRequest(getNoteQuery, token);

    const error = new NoteNotFoundError('any-id');

    expect(response.status).toBe(400);
    expect(response.body.data.getNote).toBeNull();
